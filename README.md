...
**Requirements:** macOS 26 Tahoe or newer, Xcode 26 or newer (for the macOS 26 SDK).

```bash
git clone https://github.com/<your-user>/floatube.git
cd floatube
open floatube.xcodeproj          # open in Xcode, or:

# Build a Release .app from the command line:
xcodebuild \
  -project floatube.xcodeproj \
  -scheme flotube \
...
open build/Build/Products/Release/floatube.app
```

The Swift Package dependency [YouTubePlayerKit](https://github.com/SvenTiigi/YouTubePlayerKit) resolves automatically.

---

## Releases (for maintainers)

`.github/workflows/release.yml` produces a signed `.dmg` on every `v*.*.*` tag push and attaches it to a GitHub Release.
...
The pipeline supports two signing modes:

- **Stable signed (recommended)** — set repo secrets `MAC_CERT_BASE64` and `MAC_CERT_PASSWORD` (a self-signed code-signing cert in base64 + its export password). The same cert is reused across releases, so the app's Designated Requirement stays constant — Keychain ACLs keep matching across updates and your users **don't** see a re-prompt every release.
- **Ad-hoc fallback** — if those secrets aren't set, the workflow falls back to ad-hoc signing. The DMG still works, but Keychain prompts on every update.

<details>
<summary><strong>One-time cert generation</strong></summary>

Run locally, save the `.p12` + password to a password manager, paste base64 + password into repo secrets.

```bash
# 1. Generate a 10-year self-signed code-signing cert.
TMP=$(mktemp -d)
...
openssl req -x509 -newkey rsa:2048 \
  ...
  -days 3650 -nodes -config "$TMP/req.conf" -extensions v3_ext

# 2. Pack into PKCS12, base64 the result.
read -s -p "Choose export password: " P12PASS; echo
openssl pkcs12 -export \
  -out "$TMP/floatube-cert.p12" \
  -inkey "$TMP/key.pem" -in "$TMP/cert.pem" \
  -name "Floatube Releases" \
  -passout "pass:$P12PASS"
base64 -i "$TMP/floatube-cert.p12" | pbcopy   # base64 now on clipboard
```

In the GitHub repo: **Settings → Secrets and variables → Actions → New repository secret** — add `MAC_CERT_BASE64` (paste) and `MAC_CERT_PASSWORD` (the export password).

> **Back up the `.p12` and the password.** If you lose them and regenerate, your users will see a fresh round of Keychain prompts because the new cert has a different fingerprint.
